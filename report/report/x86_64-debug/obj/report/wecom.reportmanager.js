...
plugin.wecom.reportmanager = wecom.reportmanager || function (start, item, app) {
    this.createNode("div", null, null, "item-node");
    innovaphone.lib1.loadCss(item.uri + ".css");

    var colorSchemes = {
        dark: {
            "--wecom-report-item": "#333333",
            "--wecom-report-item-text": "#f2f5f6",
            "--wecom-report-c1": "#efefef",
            "--wecom-report-c2": "#939393",
            "--wecom-report-highlight-bg": "#595959",
            "--wecom-report-input": "#191919",
            "--wecom-report-input-text": "#f2f5f6",
            "--wecom-report-button": "#191919",
            "--wecom-report-button-text": "#f2f5f6",
            "--wecom-report-button-bg": "#3D3D3D",
            "--wecom-report-green": "#7cb270",
        },
        light: {
            "--wecom-report-item": "#e9eef1",
            "--wecom-report-item-text": "#4a4a4a",
            "--wecom-report-c1": "#444444",
            "--wecom-report-c2": "#777777",
            "--wecom-report-highlight-bg": "#eaeaea",
            "--wecom-report-input": "white",
            "--wecom-report-input-text": "#4a4a49",
            "--wecom-report-button": "white",
            "--wecom-report-button-text": "#4a4a49",
            "--wecom-report-button-bg": "#CCCCCC",
            "--wecom-report-green": "#7cb270",
        }
    };
    ...
    var texts, add, reportList, templatesList, templatesListCn, instance;

    var panel = this.add(new innovaphone.ui1.Div(null, null, "wecom-report-panel"));
    var src = new app.Src(pbx);
    var typeText = ["report", "reportadmin"];
    var typeUrl = ["/wecom-report", "/wecom-reportadmin"];
    var typeCheckmarks = [
    ...
    innovaphone.lib1.loadObjectScript(item.uri + "texts", function () {
        texts = new innovaphone.lib1.Languages(wecom.reportmanagertexts, start.lang);
    ...
    function read() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("display:flex; flex-direction:row;", null, "wecom-report-obj")).addEvent("click", onadd).testId("wecom-report-add");
        add = header.add(new innovaphone.ui1.SvgInline("position:relative; left:10px; width:20px; top:10px; height:20px; fill:var(--wecom-report-item-text); cursor:pointer", "0 0 20 20", "<path d=\'M8.24,8.24V0h3.52V8.24H20v3.52H11.76V20H8.24V11.76H0V8.24Z'/>"));
        header.add(new innovaphone.ui1.Div("padding: 5px 10px;", null, "wecom-report-label2")).addTranslation(texts, "addapp");
        reportList = panel.add(new innovaphone.ui1.Scrolling("position:absolute; left:0px; right:0px; top:60px; bottom:0px", -1, -1));
        copyPwd = null;
    ...
    function onadd() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-report-c2); font-size: 18px;")).addTranslation(texts, "addapp");
        ...
        addSelect(select, 0, "report", "/wecom-report.png");
        addSelect(select, 1, "reportadmin", "/wecom-reportadmin.png");
        ...
        function addSelect(select, typeIndex, appid, iconpath) {
            ...
            var appselect = select.add(new innovaphone.ui1.Div("width:170px;height:50px;", null, "wecom-report-choice")).addEvent("click", function () {
                select.clear();
                selection.add(new innovaphone.ui1.Div("text-align: left; background-color: transparent; font-size: 16px;", null, "wecom-report-selection")).addTranslation(texts, appid);
                new Editreport({ type: typeIndex }, content);
            }).testId("wecom-report-" + appid);
            var appicon = appselect.add(new innovaphone.ui1.Div(null, null, "wecom-report-appicon"));
            ...
            appselect.add(new innovaphone.ui1.Div("position:absolute; left:50px; top:5px; height:30px;", null, "wecom-report-label2")).addTranslation(texts, appid);
        }
    }
    ...
    function pbx(msg) {
        if (msg.mt == "GetAppObjectsResult") {
            for (var i = 0; i < msg.objects.length; i++) {
                reportList.add(new report(msg.objects[i]));
            }
            templatesList = [];
            templatesListCn = [];
    ...
    function report(obj) {
        ...
        this.createNode("div", null, null, "wecom-report-obj").testId("wecom-report-obj-" + obj.sip);
        this.addEvent("click", onedit);
        var appName = this.add(new innovaphone.ui1.Div("width:100px; font-size:15px; color:var(--wecom-report-item-text);", null, "wecom-report-label2")).addTranslation(texts, typeText[obj.type]);
        var header = this.add(new innovaphone.ui1.Div(null, null, "wecom-report-header"));
        ...
        function result(msg) {
            ...
            function check(msg) {
                if (msg.ok) {
                    ...
                    header.add(new innovaphone.ui1.SvgInline("position:relative; width:20px; height:20px; margin:5px; fill:var(--wecom-report-green)", "0 0 20 20", "<path d=\'M6.67,17.5,0,10.81,1.62,9.18l5.05,5.06L18.38,2.5,20,4.13Z'/>"));
                }
            }
        }
        function onedit() {
            panel.clear();
            var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-report-c2); font-size: 18px;")).addTranslation(texts, "editapp");
            var content = panel.add(new innovaphone.ui1.Scrolling("position:absolute; width:100%; top:50px; bottom:40px; margin-top: 5px;", -1, -1, 9, "red"));
            new Editreport(obj, content);
        }
    }
    report.prototype = innovaphone.ui1.nodePrototype;

    function Editreport(obj, content) {
        var footer = panel.add(new innovaphone.ui1.Div("position:absolute; width:100%; bottom:0px; height:40px"));
        if (obj.guid) footer.add(new innovaphone.ui1.Div("left:10px; bottom:10px", null, "wecom-report-button")).addTranslation(texts, "del").addEvent("click", ondel).testId("wecom-report-del");
        footer.add(new innovaphone.ui1.Div("right:140px; bottom:10px", null, "wecom-report-button")).addTranslation(texts, "ok").addEvent("click", onok).testId("wecom-report-ok");
        footer.add(new innovaphone.ui1.Div("right:10px; bottom:10px", null, "wecom-report-button")).addTranslation(texts, "cancel").addEvent("click", oncancel).testId("wecom-report-cancel");

        var general = content.add(new innovaphone.ui1.Div("position:relative; display:flex; flex-wrap: wrap;"));
        var title = general.add(new ConfigText("title", obj.title, 150)).testId("wecom-report-cn");
        var sip = general.add(new ConfigText("sip", obj.sip, 150)).testId("wecom-report-sip");
        var tmp = [];
        var tmpSelected = [];
        for (var i = 0; i < templatesList.length; i++) {
            tmp[i] = general.add(new ConfigTemplate(obj.sip, templatesList[i])).testId("wecom-report-temp-" + templatesList[i].cn);
        }
    ...
    Editreport.prototype = innovaphone.ui1.nodePrototype;

    function Text(label, text, width, lwidth) {
        this.createNode("div", "position:relative; display:flex");
        this.add(new innovaphone.ui1.Div(lwidth ? "width:" + lwidth + "px" : null, null, "wecom-report-label")).addTranslation(texts, label);
        var text = this.add(new innovaphone.ui1.Div("width:" + width + "px", text, "wecom-report-value"));
    ...
    function ConfigText(label, text, width) {
        this.createNode("div", "position:relative; display:flex");
        var label = this.add(new innovaphone.ui1.Div(null, null, "wecom-report-label")).addTranslation(texts, label);
        var inputDiv = this.add(new innovaphone.ui1.Div("position:relative; width:" + width + "px"));
        var input = inputDiv.add(new innovaphone.ui1.Input(null, text, null, 100, null, "wecom-report-input"));
    ...
    function ConfigTemplate(sip, template) {
        ...
        var checkbox = this.add(new innovaphone.ui1.Checkbox("position:relative; margin: 7px 0px 7px 15px; width: 20px; height:20px; background-color:var(--wecom-report-green);", false, null, "var(--wecom-report-green)", "white", "var(--wecom-report-c1)"));
        var label = this.add(new innovaphone.ui1.Div("padding: 3px 5px 3px 0px;", template.cn, "wecom-report-label"));
...
plugin.wecom.reportmanager.prototype = innovaphone.ui1.nodePrototype;
