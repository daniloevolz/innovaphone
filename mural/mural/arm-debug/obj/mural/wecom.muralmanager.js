...
plugin.wecom.muralmanager = plugin.wecom.muralmanager || function (start, item, app) {
    this.createNode("div", null, null, "item-node");
    innovaphone.lib1.loadCss(item.uri + ".css");

    var colorSchemes = {
        dark: {
            "--wecom-mural-item": "#333333",
            "--wecom-mural-item-text": "#f2f5f6",
            "--wecom-mural-c1": "#efefef",
            "--wecom-mural-c2": "#939393",
            "--wecom-mural-highlight-bg": "#595959",
            "--wecom-mural-input": "#191919",
            "--wecom-mural-input-text": "#f2f5f6",
            "--wecom-mural-button": "#191919",
            "--wecom-mural-button-text": "#f2f5f6",
            "--wecom-mural-button-bg": "#3D3D3D",
            "--wecom-mural-green": "#7cb270",
        },
        light: {
            "--wecom-mural-item": "#e9eef1",
            "--wecom-mural-item-text": "#4a4a4a",
            "--wecom-mural-c1": "#444444",
            "--wecom-mural-c2": "#777777",
            "--wecom-mural-highlight-bg": "#eaeaea",
            "--wecom-mural-input": "white",
            "--wecom-mural-input-text": "#4a4a49",
            "--wecom-mural-button": "white",
            "--wecom-mural-button-text": "#4a4a49",
            "--wecom-mural-button-bg": "#CCCCCC",
            "--wecom-mural-green": "#7cb270",
        }
    };
    ...
    var texts, add, muralList, templatesList, templatesListCn, instance;

    var panel = this.add(new innovaphone.ui1.Div(null, null, "wecom-mural-panel"));
    var src = new app.Src(pbx);
    var typeText = ["mural", "muraladmin"];
    var typeUrl = ["/wecom-mural", "/wecom-muraladmin"];
    var typeCheckmarks = [
    ...
    innovaphone.lib1.loadObjectScript(item.uri + "texts", function () {
        texts = new innovaphone.lib1.Languages(wecom.muralmanagertexts, start.lang);
    ...
    function read() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("display:flex; flex-direction:row;", null, "wecom-mural-obj")).addEvent("click", onadd).testId("wecom-mural-add");
        add = header.add(new innovaphone.ui1.SvgInline("position:relative; left:10px; width:20px; top:10px; height:20px; fill:var(--wecom-mural-item-text); cursor:pointer", "0 0 20 20", "<path d=\'M8.24,8.24V0h3.52V8.24H20v3.52H11.76V20H8.24V11.76H0V8.24Z'/>"));
        header.add(new innovaphone.ui1.Div("padding: 5px 10px;", null, "wecom-mural-label2")).addTranslation(texts, "addapp");
        muralList = panel.add(new innovaphone.ui1.Scrolling("position:absolute; left:0px; right:0px; top:60px; bottom:0px", -1, -1));
        copyPwd = null;
    ...
    function onadd() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-mural-c2); font-size: 18px;")).addTranslation(texts, "addapp");
        ...
        addSelect(select, 0, "mural", "/wecom-mural.png");
        addSelect(select, 1, "muraladmin", "/wecom-muraladmin.png");
        ...
        function addSelect(select, typeIndex, appid, iconpath) {
            ...
            var appselect = select.add(new innovaphone.ui1.Div("width:170px;height:50px;", null, "wecom-mural-choice")).addEvent("click", function () {
                select.clear();
                selection.add(new innovaphone.ui1.Div("text-align: left; background-color: transparent; font-size: 16px;", null, "wecom-mural-selection")).addTranslation(texts, appid);
                new Editmural({ type: typeIndex }, content);
            }).testId("wecom-mural-" + appid);
            var appicon = appselect.add(new innovaphone.ui1.Div(null, null, "wecom-mural-appicon"));
            ...
            appselect.add(new innovaphone.ui1.Div("position:absolute; left:50px; top:5px; height:30px;", null, "wecom-mural-label2")).addTranslation(texts, appid);
        }
    }
    ...
    function pbx(msg) {
        if (msg.mt == "GetAppObjectsResult") {
            for (var i = 0; i < msg.objects.length; i++) {
                muralList.add(new mural(msg.objects[i]));
            }
            templatesList = [];
            templatesListCn = [];
    ...
    function mural(obj) {
        ...
        this.createNode("div", null, null, "wecom-mural-obj").testId("wecom-mural-obj-" + obj.sip);
        this.addEvent("click", onedit);
        var appName = this.add(new innovaphone.ui1.Div("width:100px; font-size:15px; color:var(--wecom-mural-item-text);", null, "wecom-mural-label2")).addTranslation(texts, typeText[obj.type]);
        var header = this.add(new innovaphone.ui1.Div(null, null, "wecom-mural-header"));
        ...
        function result(msg) {
            ...
            function check(msg) {
                if (msg.ok) {
                    ...
                    header.add(new innovaphone.ui1.SvgInline("position:relative; width:20px; height:20px; margin:5px; fill:var(--wecom-mural-green)", "0 0 20 20", "<path d=\'M6.67,17.5,0,10.81,1.62,9.18l5.05,5.06L18.38,2.5,20,4.13Z'/>"));
                }
            }
        }
        function onedit() {
            panel.clear();
            var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-mural-c2); font-size: 18px;")).addTranslation(texts, "editapp");
            var content = panel.add(new innovaphone.ui1.Scrolling("position:absolute; width:100%; top:50px; bottom:40px; margin-top: 5px;", -1, -1, 9, "red"));
            new Editmural(obj, content);
        }
    }
    mural.prototype = innovaphone.ui1.nodePrototype;

    function Editmural(obj, content) {
        var footer = panel.add(new innovaphone.ui1.Div("position:absolute; width:100%; bottom:0px; height:40px"));
        if (obj.guid) footer.add(new innovaphone.ui1.Div("left:10px; bottom:10px", null, "wecom-mural-button")).addTranslation(texts, "del").addEvent("click", ondel).testId("wecom-mural-del");
        footer.add(new innovaphone.ui1.Div("right:140px; bottom:10px", null, "wecom-mural-button")).addTranslation(texts, "ok").addEvent("click", onok).testId("wecom-mural-ok");
        footer.add(new innovaphone.ui1.Div("right:10px; bottom:10px", null, "wecom-mural-button")).addTranslation(texts, "cancel").addEvent("click", oncancel).testId("wecom-mural-cancel");

        var general = content.add(new innovaphone.ui1.Div("position:relative; display:flex; flex-wrap: wrap;"));
        var title = general.add(new ConfigText("title", obj.title, 150)).testId("wecom-mural-cn");
        var sip = general.add(new ConfigText("sip", obj.sip, 150)).testId("wecom-mural-sip");
        var tmp = [];
        var tmpSelected = [];
        for (var i = 0; i < templatesList.length; i++) {
            tmp[i] = general.add(new ConfigTemplate(obj.sip, templatesList[i])).testId("wecom-mural-temp-" + templatesList[i].cn);
        }
    ...
    Editmural.prototype = innovaphone.ui1.nodePrototype;

    function Text(label, text, width, lwidth) {
        this.createNode("div", "position:relative; display:flex");
        this.add(new innovaphone.ui1.Div(lwidth ? "width:" + lwidth + "px" : null, null, "wecom-mural-label")).addTranslation(texts, label);
        var text = this.add(new innovaphone.ui1.Div("width:" + width + "px", text, "wecom-mural-value"));
    ...
    function ConfigText(label, text, width) {
        this.createNode("div", "position:relative; display:flex");
        var label = this.add(new innovaphone.ui1.Div(null, null, "wecom-mural-label")).addTranslation(texts, label);
        var inputDiv = this.add(new innovaphone.ui1.Div("position:relative; width:" + width + "px"));
        var input = inputDiv.add(new innovaphone.ui1.Input(null, text, null, 100, null, "wecom-mural-input"));
    ...
    function ConfigTemplate(sip, template) {
        ...
        var checkbox = this.add(new innovaphone.ui1.Checkbox("position:relative; margin: 7px 0px 7px 15px; width: 20px; height:20px; background-color:var(--wecom-mural-green);", false, null, "var(--wecom-mural-green)", "white", "var(--wecom-mural-c1)"));
        var label = this.add(new innovaphone.ui1.Div("padding: 3px 5px 3px 0px;", template.cn, "wecom-mural-label"));
...
plugin.wecom.muralmanager.prototype = innovaphone.ui1.nodePrototype;
