...
innovaphone.ui1.Editor = innovaphone.ui1.Editor || function (style, scrollWidth, scrollColor, scrollStyle, onCtrlEnter, onEsc, onChanged, onFocus, onEmphasis) {
    ...
    function focus(n, s) {
        ...
        s.collapse(n, 0);
        var s = n.offsetTop + n.offsetHeight - that.container.offsetHeight;
        if (s > that.container.scrollTop) that.container.scrollTop = s + 5;
    }
    ...
    function normalize(node, parent, before) {
        ...
        for (n = node.firstChild; n;) {
            var p = node;
            if (['B', 'I', 'U', 'P', 'UL', 'OL', 'DL', 'LI', 'DT', 'DD', 'DIV', 'BR'].indexOf(n.nodeName) != -1) {
                p = document.createElement(n.nodeName);
                parent.insertBefore(p, parent == node ? n : before);
                before = null;
                normalize(n, p, before);
            }
            else if (['SPAN', 'A'].indexOf(n.nodeName) != -1) {
                normalize(n, parent, before);
            }
    ...
    function split(p, ofs) {
        ...
        for (var j = 1; j < p.length; j++) {
            var i;
            if (p[j].nodeType == 3) {
                i = c.appendChild(document.createTextNode(p[j].wholeText.slice(ofs)));
                p[j].textContent = p[j].wholeText.slice(0, ofs);
                if (ofs == 0) p[j].parentNode.innerHTML = "<br/>";
            }
            else {
    ...
    function empty(node, next) {
        if (node.nodeType == 3 && node.textContent.length && node.textContent.search(/\S/) != -1) return false;
        if (node.firstChild) {
    ...
    function unindentli(p) {
        ...
        if (p[0].parentNode.parentNode.nodeName == "LI") {
            ...
            var e = nextNode(p[0].parentNode.parentNode.tagName)
            p[0].parentNode.parentNode.insertBefore(e, p[0].parentNode.nextSibling);
            p[0].parentNode.removeChild(p[0]);
            window.getSelection().collapse(e);
        }
    }

    function nextNode(parent) {
        var tag = "P";
        if (parent == "OL" || parent == "UL") tag = "LI";
        else if (parent == "DL") tag = "DD";
    ...
    this.selection = function (tag) {
        ...
        function traverse(start, end, sofs, eofs, op) {
            ...
            while (cur != end) {
                ...
                if (next) cur = next;
                next = null;
                if (cur.firstChild && !up) { cur = cur.firstChild; }
                else if (cur.nextSibling) { cur = cur.nextSibling; up = false }
                else { cur = cur.parentNode; up = true; };
    ...
    this.insert = function (text) {
        var s = selection();
        if (!s) return;
        s.f.textContent = s.f.textContent.slice(0, s.o) + text + s.f.textContent.slice(s.o);
    ...
    this.setText = function (html) { content.addHTML(html); if (onChanged) onChanged(); return this };
