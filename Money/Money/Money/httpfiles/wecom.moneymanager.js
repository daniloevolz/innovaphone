...
plugin.wecom.moneymanager = wecom.moneymanager || function (start, item, app) {
    this.createNode("div", null, null, "item-node");
    innovaphone.lib1.loadCss(item.uri + ".css");

    var colorSchemes = {
        dark: {
            "--wecom-money-item": "#333333",
            "--wecom-money-item-text": "#f2f5f6",
            "--wecom-money-c1": "#efefef",
            "--wecom-money-c2": "#939393",
            "--wecom-money-highlight-bg": "#595959",
            "--wecom-money-input": "#191919",
            "--wecom-money-input-text": "#f2f5f6",
            "--wecom-money-button": "#191919",
            "--wecom-money-button-text": "#f2f5f6",
            "--wecom-money-button-bg": "#3D3D3D",
            "--wecom-money-green": "#7cb270",
        },
        light: {
            "--wecom-money-item": "#e9eef1",
            "--wecom-money-item-text": "#4a4a4a",
            "--wecom-money-c1": "#444444",
            "--wecom-money-c2": "#777777",
            "--wecom-money-highlight-bg": "#eaeaea",
            "--wecom-money-input": "white",
            "--wecom-money-input-text": "#4a4a49",
            "--wecom-money-button": "white",
            "--wecom-money-button-text": "#4a4a49",
            "--wecom-money-button-bg": "#CCCCCC",
            "--wecom-money-green": "#7cb270",
        }
    };
    ...
    var texts, add, MoneyList, templatesList, templatesListCn, instance;

    var panel = this.add(new innovaphone.ui1.Div(null, null, "wecom-money-panel"));
    var src = new app.Src(pbx);
    var typeText = ["money", "moneyadmin"];
    var typeUrl = ["/wecom-money", "/wecom-moneyadmin"];
    var typeCheckmarks = [
    ...
    innovaphone.lib1.loadObjectScript(item.uri + "texts", function () {
        texts = new innovaphone.lib1.Languages(wecom.moneymanagertexts, start.lang);
    ...
    function read() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("display:flex; flex-direction:row;", null, "wecom-money-obj")).addEvent("click", onadd).testId("wecom-money-add");
        add = header.add(new innovaphone.ui1.SvgInline("position:relative; left:10px; width:20px; top:10px; height:20px; fill:var(--wecom-money-item-text); cursor:pointer", "0 0 20 20", "<path d=\'M8.24,8.24V0h3.52V8.24H20v3.52H11.76V20H8.24V11.76H0V8.24Z'/>"));
        header.add(new innovaphone.ui1.Div("padding: 5px 10px;", null, "wecom-money-label2")).addTranslation(texts, "addapp");
        MoneyList = panel.add(new innovaphone.ui1.Scrolling("position:absolute; left:0px; right:0px; top:60px; bottom:0px", -1, -1));
        copyPwd = null;
    ...
    function onadd() {
        panel.clear();
        var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-money-c2); font-size: 18px;")).addTranslation(texts, "addapp");
        ...
        addSelect(select, 0, "money", "/wecom-money.png");
        addSelect(select, 1, "moneyadmin", "/wecom-moneyadmin.png");
        ...
        function addSelect(select, typeIndex, appid, iconpath) {
            ...
            var appselect = select.add(new innovaphone.ui1.Div("width:170px;height:50px;", null, "wecom-money-choice")).addEvent("click", function () {
                select.clear();
                selection.add(new innovaphone.ui1.Div("text-align: left; background-color: transparent; font-size: 16px;", null, "wecom-money-selection")).addTranslation(texts, appid);
                new EditMoney({ type: typeIndex }, content);
            }).testId("wecom-money-" + appid);
            var appicon = appselect.add(new innovaphone.ui1.Div(null, null, "wecom-money-appicon"));
            ...
            appselect.add(new innovaphone.ui1.Div("position:absolute; left:50px; top:5px; height:30px;", null, "wecom-money-label2")).addTranslation(texts, appid);
        }
    }
    ...
    function pbx(msg) {
        if (msg.mt == "GetAppObjectsResult") {
            for (var i = 0; i < msg.objects.length; i++) {
                MoneyList.add(new Money(msg.objects[i]));
            }
            templatesList = [];
            templatesListCn = [];
    ...
    function Money(obj) {
        ...
        this.createNode("div", null, null, "wecom-money-obj").testId("wecom-money-obj-" + obj.sip);
        this.addEvent("click", onedit);
        var appName = this.add(new innovaphone.ui1.Div("width:100px; font-size:15px; color:var(--wecom-money-item-text);", null, "wecom-money-label2")).addTranslation(texts, typeText[obj.type]);
        var header = this.add(new innovaphone.ui1.Div(null, null, "wecom-money-header"));
        ...
        function result(msg) {
            ...
            function check(msg) {
                if (msg.ok) {
                    ...
                    header.add(new innovaphone.ui1.SvgInline("position:relative; width:20px; height:20px; margin:5px; fill:var(--wecom-money-green)", "0 0 20 20", "<path d=\'M6.67,17.5,0,10.81,1.62,9.18l5.05,5.06L18.38,2.5,20,4.13Z'/>"));
                }
            }
        }
        function onedit() {
            panel.clear();
            var header = panel.add(new innovaphone.ui1.Div("position:absolute; box-sizing:border-box; padding:10px; width:100%; color: var(--wecom-money-c2); font-size: 18px;")).addTranslation(texts, "editapp");
            var content = panel.add(new innovaphone.ui1.Scrolling("position:absolute; width:100%; top:50px; bottom:40px; margin-top: 5px;", -1, -1, 9, "red"));
            new EditMoney(obj, content);
        }
    }
    Money.prototype = innovaphone.ui1.nodePrototype;

    function EditMoney(obj, content) {
        var footer = panel.add(new innovaphone.ui1.Div("position:absolute; width:100%; bottom:0px; height:40px"));
        if (obj.guid) footer.add(new innovaphone.ui1.Div("left:10px; bottom:10px", null, "wecom-money-button")).addTranslation(texts, "del").addEvent("click", ondel).testId("wecom-money-del");
        footer.add(new innovaphone.ui1.Div("right:140px; bottom:10px", null, "wecom-money-button")).addTranslation(texts, "ok").addEvent("click", onok).testId("wecom-money-ok");
        footer.add(new innovaphone.ui1.Div("right:10px; bottom:10px", null, "wecom-money-button")).addTranslation(texts, "cancel").addEvent("click", oncancel).testId("wecom-money-cancel");

        var general = content.add(new innovaphone.ui1.Div("position:relative; display:flex; flex-wrap: wrap;"));
        var title = general.add(new ConfigText("title", obj.title, 150)).testId("wecom-money-cn");
        var sip = general.add(new ConfigText("sip", obj.sip, 150)).testId("wecom-money-sip");
        var tmp = [];
        var tmpSelected = [];
        for (var i = 0; i < templatesList.length; i++) {
            tmp[i] = general.add(new ConfigTemplate(obj.sip, templatesList[i])).testId("wecom-money-temp-" + templatesList[i].cn);
        }
    ...
    EditMoney.prototype = innovaphone.ui1.nodePrototype;

    function Text(label, text, width, lwidth) {
        this.createNode("div", "position:relative; display:flex");
        this.add(new innovaphone.ui1.Div(lwidth ? "width:" + lwidth + "px" : null, null, "wecom-money-label")).addTranslation(texts, label);
        var text = this.add(new innovaphone.ui1.Div("width:" + width + "px", text, "wecom-money-value"));
    ...
    function ConfigText(label, text, width) {
        this.createNode("div", "position:relative; display:flex");
        var label = this.add(new innovaphone.ui1.Div(null, null, "wecom-money-label")).addTranslation(texts, label);
        var inputDiv = this.add(new innovaphone.ui1.Div("position:relative; width:" + width + "px"));
        var input = inputDiv.add(new innovaphone.ui1.Input(null, text, null, 100, null, "wecom-money-input"));
    ...
    function ConfigTemplate(sip, template) {
        ...
        var checkbox = this.add(new innovaphone.ui1.Checkbox("position:relative; margin: 7px 0px 7px 15px; width: 20px; height:20px; background-color:var(--wecom-money-green);", false, null, "var(--wecom-money-green)", "white", "var(--wecom-money-c1)"));
        var label = this.add(new innovaphone.ui1.Div("padding: 3px 5px 3px 0px;", template.cn, "wecom-money-label"));
...
plugin.wecom.moneymanager.prototype = innovaphone.ui1.nodePrototype;
